...
namespace BlazorShortUrl.Controllers
{
    ...
    public class IdentityController : Controller
    {
        ...
        [HttpGet("[action]")]
        public Dictionary<string, string?> RolesList()
        {
            Console.WriteLine("IdentityController => RolesList");
            return _roles;
        }

        [HttpGet("[action]")]
        // public Dictionary<string, string> ClaimsList() => _claimTypes;
        public async Task<Dictionary<string, string?>> ClaimsList()
        {
        ...
        public async Task<dynamic> RoleList()
        {
            ...
            var data = (await qry.ToArrayAsync()).Select(r => new
            {
                r.Id,
                r.Name,
                Claims = r.Claims.Select(c =>
                    new KeyValuePair<string, string>(_claimTypes.Single(x => x.Value == c.ClaimType).Key,
                        c.ClaimValue)),
            });
